...
extern "C" {
uint16_t get_engine_move(Board board, uint8_t search_depth, uint16_t last_move,
                         bool is_team_white);

bool is_in_check(Board *board, bool is_checking_white);
void get_piece_legal_moves(Board *board, uint16_t *array_ptr,
                           uint16_t last_move, uint8_t cell, uint8_t castle);
void get_strictly_legal_moves(Board *board, uint16_t last_move,
                              uint16_t *array_ptr);

bool try_make_move_and_check_if_causes_check(Board *board, uint16_t last_move,
                                             uint16_t move_data);
